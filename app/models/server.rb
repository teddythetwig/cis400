class Server < ActiveRecord::Base
  ...
  after_create do |s|
    ...
    $RDS.create_db_instance(:db_instance_identifier => "#{name}-#{Rails.env}", :allocated_storage => 5,:db_instance_class => "db.t1.micro",:engine => "MySQL",:master_username=>"username#{user.id}",:master_user_password=>"cis400")
